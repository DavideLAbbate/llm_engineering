...
from orchestrator import run_multi_response_pipeline
from verifier import run_advanced_verification
from chatbot_utils import get_claude_response, get_openai_response

# --- Legacy classes for logging (can be refactored later) ---
class Reasoning:
...
def run_simple_workflow(user_prompt: str, history: list):
    ...
    # Simple verification can be added here if needed, but the prompt focuses on multi-part
...
def chat_stream(user_input: str, history: list):
    ...
    if analysis.get("numberOfResponse", 1) > 10:
        yield history + [[user_input, "Errore: la richiesta è troppo complessa e richiede troppi passaggi. Chiedi aiuto a un amministratore di sistema."]], "Errore"
        return

    # Check if we are continuing a task or starting a new one
    ...
    if analysis.get("outputRequireMultipleResponses"):
        # FEATURE 2 & 3: Multi-Response Workflow
        if is_new_task:
...
def main():
    with gr.Blocks(theme=gr.themes.Soft(), css="body {background: #181A20;}") as demo:
        gr.Markdown("<h1 style='color:#fff;'>Sara - AI Chatbot v7</h1><p style='color:#bbb;'>Assistant con orchestrazione multi-risposta e verifica avanzata.</p>")
