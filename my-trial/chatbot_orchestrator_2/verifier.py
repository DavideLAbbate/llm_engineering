...
from chatbot_utils import get_claude_response, get_openai_response # We'll create this file
...
def verify_single_part(part_number, history, flowchart):
    ...
    # For simplicity, we'll generate contextual descriptions on the fly.
    # A more advanced implementation might generate these during the orchestration phase.
    ...
    with open("prompts/multi_part_verifier_prompt.txt", "r", encoding="utf-8") as f:
        verifier_prompt_template = f.read()

    # This is a simplified representation. The actual prompt would be more dynamic.
    system_prompt = verifier_prompt_template # The prompt itself acts as the system message
    user_prompt = f"""
